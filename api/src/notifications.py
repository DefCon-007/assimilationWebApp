...
def sendNotification(grp,date,title) :
# Send to multiple devices by passing a list of ids.
    ...
    message_body = f"On {date}. {title}"
    if valid_registration_ids :
        result = push_service.notify_multiple_devices(registration_ids=valid_registration_ids, message_title=message_title, message_body=message_body)
        print (result)
    # print (result)